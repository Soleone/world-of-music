  ...
  module Formtastic #:nodoc:
    ...
    class SemanticFormBuilder < ActionView::Helpers::FormBuilder
      ...
      def input(method, options = {})
        ...
        return @template.content_tag(:div, content, list_item_html_attributes(method, options))
      end
      ...
      def select_input(method, options)
        ...
        # TODO add option?!?!
        # choices = options[:empty_element] ? [options[:empty_element]] : []
        choices = options[:add_empty] ? [["none", nil]] : []
      ...
      def date_or_datetime_input(method, options)
        ...
        (inputs + time_inputs).each do |input|
          if options["discard_#{input}".intern]
            ...
            list_items_capture << @template.content_tag(:label, input.to_s.humanize, :for => "#{@object_name}_#{method}_#{position[input]}i") + 
        ...
        @template.content_tag(:fieldset, 
          %{<legend><span>#{label_text(method, options)}</span></legend>} + 
          @template.content_tag(:div, list_items_capture, :class => "date")
        )
      end
      ...
      def boolean_radio_input(method, options)
        ...
        @template.content_tag(:fieldset, 
          %{<legend><span>#{label_text(method, options)}</span></legend>} + 
              # TODO checked is always false, which ain't always right ;)
          @template.content_tag(:div, 
            choices.map { |c| 
              @template.label_tag("#{@object_name}_#{method}_#{c[:value]}", 
                "#{@template.radio_button_tag("#{@object_name}[#{method}]", c[:value], @template.instance_eval("@#{@object_name}").send(method) == c[:value], :id => "#{@object_name}_#{method}_#{c[:value]}")} #{c[:label]}"
              )
            },
            :class => "booleans"
          )
